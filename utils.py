...
Left_iris = [380, 381, 382, 384, 385,386, 387, 388, 390, 398, 466 ] 
Right_iris = []


Left_shadow = [190,189,56,221,28,222,27,223,29,224,30,225,113,247,246]
# Right_shadow = [359,446,467,342,260,445,259,444,257,443,258,442,286,441,414,413]
...
def apply_makeup(src: np.ndarray, is_stream: bool, feature: str, show_landmarks: bool = False):
    ...
    if feature == 'lips':
        ...
        mask_left = shadow_mask(src,feature_landmarks_left,[0, 0, 100])
        feature_landmarks_right = normalize_landmarks(ret_landmarks,height,width,Right_shadow)
        mask_right = shadow_mask(src,feature_landmarks_right,[0, 0, 100])
        mask  = mask_right+mask_left
        output = cv2.addWeighted(src,1.0,mask,0.4, 0.0)
    elif feature =='eye':
        feature_landmarks = normalize_landmarks(ret_landmarks,height,width,Right_iris+Left_iris)
        mask = eye_mask(src,feature_landmarks,[])
        output = cv2.addWeighted(src,1.0,mask,0.4, 0.0)
    elif feature == 'blush':
        feature_landmarks_right = normalize_landmarks(ret_landmarks, height, width, right_blush)
        mask_right = blush_mask(src, feature_landmarks_right, [0, 0, 100],75)#
        feature_landmarks_left = normalize_landmarks(ret_landmarks, height, width, left_blush)
        mask_left = blush_mask(src, feature_landmarks_left, [0, 0, 100],75)
        mask = mask_left+mask_right
        output = cv2.addWeighted(src, 1.0, mask, 0.1, 0.0)
...
def apply_feature(src: np.ndarray, feature: str, landmarks: list, normalize: bool = False,
    ...
    if feature == 'lips':
        ...
        mask = blush_mask(src, landmarks, [83, 80, 125], 20) 
...
def eye_mask(src: np.ndarray, points: np.ndarray, color: list):
    """
    Given a src image, points of lips and a desired color
    Returns a colored mask that can be added to the src
    """
